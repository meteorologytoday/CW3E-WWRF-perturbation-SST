...
if __name__ == "__main__":
    ...
    std_precip_levs = np.linspace(0, args.std_precip_max, 11)
    ...
    for i, da in enumerate(data):
        ...
        if is_ref: #
            ...
            mappable = _ax.contourf(lon, lat, da.sel(stat="std"), std_precip_levs, cmap="cmo.rain", transform=proj, extend="max")
            cax = tool_fig_config.addAxesNextToAxes(fig, _ax, "right", thickness=0.03, spacing=0.05)
            cb = plt.colorbar(mappable, cax=cax, orientation="vertical", pad=0.00)
        ...
        ax_flatten[0].set_title(args.input_files[i])
